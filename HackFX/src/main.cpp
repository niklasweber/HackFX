...
void onFaderValChanged(Fader *fader)
{
	// Serial.printf("[%u=>%d]\n",fader->getId(), fader->getValue());
  // MIDI.sendControlChange(1,faderMux.readNext(),5);
  MIDI.sendControlChange(fader->getId(),fader->getValue(),5);
}
...
void setup() {
  ...
  faderMux.registerFaderChangedCallBack(onFaderValChanged);
  faderMux.init();
